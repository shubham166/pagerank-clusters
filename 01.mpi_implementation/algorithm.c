...
void compute_page_rank(rankvec *rsrc , rankvec *rdest, warray *Wg, warray *Wi, warray *Wo, int size, int N, int myrank,int it);
float absoulte_val(float val);

void page_rank(int N,char *filename)
{
	MPI_Init(NULL,NULL);
	...
	// printf("before\n");
	compute_page_rank(&rsrc,&rdest,&Wg,&Wi,&Wo,size,N,myrank,100);
